...
public class UserDao {
	...
	public static boolean insertBook(SinhVien SV) throws SQLException {
		Connection conn = Connect.getConnection();
		String sql = "INSERT INTO quan_ly_sinh_vien(userid, ten, ngay_sinh, que_quan, gioi_tinh, lop) VALUES (?, ?, ?, ?, ?, ?)";
	...
	public static boolean deleteBook(String maSinhVien) throws SQLException {
		...
		String sql = "DELETE FROM quan_ly_sinh_vien WHERE userid=?";
	...
	public static SinhVien getBook(String maSV) throws SQLException {
		...
		SinhVien SV = null;
		String sql = "SELECT * FROM quan_ly_sinh_vien WHERE userid=?";
	...
	public static boolean updateBook(SinhVien SV) throws SQLException {
		...
		String sql = "UPDATE quan_ly_sinh_vien SET ten =?, ngay_sinh =?, que_quan =?, gioi_tinh =?, lop =? WHERE userid=?";
		PreparedStatement statement = conn.prepareStatement(sql);
	...
	public static List<SinhVien> getRecords(int start, int total) {
		...
		try {
			Connection conn = Connect.getConnection();
			PreparedStatement ps = conn.prepareStatement("select * from quan_ly_sinh_vien limit " + start + "," + total);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	public static int soTrang() throws SQLException {
		int soTrang = 0;
		Connection conn = Connect.getConnection();
		String sql = "select count(userid) from quan_ly_sinh_vien";
		PreparedStatement statement = conn.prepareStatement(sql);
