...
<table class="table table-hover">
	...
	<c:forEach var = "x" items="${listSinhVien}"> 
		<tr>
			...
			<td>${x.dienThoai}</td>
			<td>${x.maLop}</td>
			<td><img src="<c:url value="/images/${x.hinhAnh}" />" width="60" height="80"></td>
			<td>
				<a href="editSv/${x.id}"><button class="btn btn-warning">Sửa</button></a>
								&nbsp; &nbsp;
				<a href="deleteSv/${x.id}"><button class="btn btn-danger" onclick="if (!confirm('Bạn có muốn xóa sinh viên này không?')) return false">Xóa</button></a>
			</td>
		</tr>
	</c:forEach>	
</table>
