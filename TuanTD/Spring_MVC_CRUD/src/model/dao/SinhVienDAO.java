...
public class SinhVienDAO {
	...
	public List<SinhVien> getAllStudent(int start, int limit) {
		String sql = "SELECT * FROM student ORDER BY MaSV ASC LIMIT "+start+","+limit;
	...
	public boolean insertStudent(SinhVien sv) {
		String sql = "INSERT INTO student ( masv, name, lop, email, address, age, avatar, author) VALUES(?,?,?,?,?,?,?,?)";
	...
	public boolean updateStudent(SinhVien sv) {
		String sql = "UPDATE student SET TenSV = ?, NamSinh = ?, Email = ?, DiaChi = ?, LopHoc = ?, Avatar = ? WHERE MaSV = ?";
		return template.execute(sql, new PreparedStatementCallback<Boolean>() {
			@Override
			public Boolean doInPreparedStatement(PreparedStatement ps) throws SQLException, DataAccessException {
